...
if __name__=='__main__':
    ...
    for num in num_threads :
        ...
        for temp in range (num):
            texec[temp] = threading.Thread(target = getdatapemain, args=(random.randint(1,14),False)) #Unsecured Connection
            texec[temp].start()
            # texec[temp].join()
        ...
        print(f"Hasil dari : {num} thread {success} response sukses dan {failed} response failed {catat_akhir - catat_awal}")
        print(f"-----------------------------''----------------------------------------")
        print("\n")
